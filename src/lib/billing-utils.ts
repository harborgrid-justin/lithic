...
export function calculateDaysInAR(claims: { dateOfService: string; paidAmount?: number }[]): number {
  const unpaidClaims = claims.filter(c => !c.paidAmount || c.paidAmount === 0);
  if (unpaidClaims.length === 0) return 0;

  const totalDays = unpaidClaims.reduce((sum, claim) => {
    const dos = new Date(claim.dateOfService);
    const today = new Date();
    const days = Math.floor((today.getTime() - dos.getTime()) / (1000 * 60 * 60 * 24));
    return sum + days;
  }, 0);
...
export function calculateCollectionRate(totalCharges: number, totalPayments: number): number {
  if (totalCharges === 0) return 0;
  return Math.round((totalPayments / totalCharges) * 100);
}

export function calculateReimbursementRate(chargedAmount: number, paidAmount: number): number {
