...
type ImmunizationRegistryConfig = z.infer<typeof ImmunizationRegistryConfigSchema>;
...
export class ImmunizationRegistryClient {
  ...
  async getImmunizationHistory(params: {
    ...
    if (this.config.hl7Enabled) {
      ...
      const response = await this.request('GET', `/immunization/history?${queryParams}`);
  ...
  async getForecast(params: {
    ...
    const response = await this.request('GET', `/immunization/forecast?${queryParams}`);
  ...
  async reportAdverseReaction(params: {
    ...
    const response = await this.request('POST', '/adverse-reaction/report', params);
  ...
  private buildVXUMessage(record: ImmunizationRecord): string {
    ...
    // MSH - Message Header
    segments.push(`MSH|^~\\&|${this.config.facilityId}|FACILITY|IIS|STATE|${timestamp}||VXU^V04^VXU_V04|${Date.now()}|P|2.5.1|||NE|NE|||||Z22^CDCPHINVS`);

    // PID - Patient Identification
    segments.push(`PID|1||^^^MPI^MR||${record.patient.lastName}^${record.patient.firstName}^${record.patient.middleName || ''}||${this.formatHL7Date(record.patient.dateOfBirth)}|${record.patient.gender}|||${record.patient.address?.street || ''}^^${record.patient.address?.city || ''}^${record.patient.address?.state || ''}^${record.patient.address?.zipCode || ''}|||||||${record.patient.ssn || ''}`);
    ...
    // ORC - Common Order
    segments.push(`ORC|RE||${Date.now()}|||||||${record.administrator.npi || ''}^${record.administrator.name}^^^^^^^NPI`);
    ...
    const rxaFields = [
      ...
      record.immunization.expirationDate ? this.formatHL7Date(record.immunization.expirationDate) : '',
    ...
    segments.push(`RXR|${record.immunization.route}^${record.immunization.route}^NCIT||${record.immunization.site}^${record.immunization.site}^HL70163`);

    // OBX - Observation (Dose Number)
    if (record.immunization.doseNumber) {
      segments.push(`OBX|1|NM|30973-2^Dose number in series^LN||${record.immunization.doseNumber}||||||F`);
    }

    // OBX - Adverse Reaction
    if (record.adverseReaction?.occurred) {
      segments.push(`OBX|2|CE|31044-1^Reaction^LN||${record.adverseReaction.description}||||||F`);
    }
  ...
  private buildQBPMessage(params: any): string {
    ...
    // MSH - Message Header
    segments.push(`MSH|^~\\&|${this.config.facilityId}|FACILITY|IIS|STATE|${timestamp}||QBP^Q11^QBP_Q11|${Date.now()}|P|2.5.1|||NE|NE|||||Z34^CDCPHINVS`);

    // QPD - Query Parameter Definition
    segments.push(`QPD|Z34^Request Immunization History^CDCPHINVS|${Date.now()}|${params.lastName}^${params.firstName}^${params.middleName || ''}^^^^L||${this.formatHL7Date(params.dateOfBirth)}|${params.gender || ''}|${params.mothersName || ''}^^^^^M`);
