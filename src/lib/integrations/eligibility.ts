...
export class EligibilityClient {
   ...
   */
  async checkBatch(requests: EligibilityRequest[]): Promise<
    Array<{
      request: EligibilityRequest;
  ...
  private buildEDI270(request: EligibilityRequest): string {
    ...
    if (request.serviceDate) {
      segments.push(
        "DTP*291*D8*" +
          request.serviceDate.toISOString().substring(0, 10).replace(/-/g, "") +
          "~",
      );
    }
  ...
  private parseEDI271(response: any): EligibilityResponse {
    ...
    return {
      ...
      terminationDate: response.terminationDate
        ? new Date(response.terminationDate)
        : undefined,
      planInfo: {
