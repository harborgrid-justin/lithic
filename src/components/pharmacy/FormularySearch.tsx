...
export function FormularySearch({ results }: FormularySearchProps) {
  const getTierBadge = (tier: number) => {
    ...
    return (
      <span
        className={`inline-flex px-2 py-1 text-xs font-semibold rounded ${colors[tier as keyof typeof colors]}`}
      >
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`inline-flex px-2 py-1 text-xs font-semibold rounded ${colors[status as keyof typeof colors]}`}
      >
        {status}
      </span>
    );
        ...
        <div key={entry.id} className="p-6 hover:bg-gray-50">
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600 mt-1">
                {entry.drug.genericName} {entry.drug.strength} -{" "}
                {entry.drug.dosageForm}
              </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 text-sm">
            <div>
              ...
              <span
                className={`ml-2 ${entry.priorAuthRequired ? "text-red-600 font-medium" : "text-gray-600"}`}
              >
            ...
            <div>
              ...
              <span
                className={`ml-2 ${entry.stepTherapyRequired ? "text-yellow-600 font-medium" : "text-gray-600"}`}
              >
