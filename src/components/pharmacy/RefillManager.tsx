...
interface RefillManagerProps {
  ...
  onProcess: (id: string, action: 'approve' | 'deny', notes?: string, denialReason?: string) => void;
  onRefresh: () => void;
}

export function RefillManager({ refillRequests, onProcess, onRefresh }: RefillManagerProps) {
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className="ml-2 text-gray-600 capitalize">{request.requestedBy}</span>
                </div>
                <div>
                ...
                <div className="mt-3 p-3 bg-gray-50 rounded">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    <div>
                      ...
                      <span className="ml-1">{request.prescription.quantity}</span>
                    </div>
                    <div>
                      <span className="font-medium text-gray-600">Days Supply:</span>
                      <span className="ml-1">{request.prescription.daysSupply}</span>
                    </div>
                    <div>
                      <span className="font-medium text-gray-600">Refills Remaining:</span>
                      <span className="ml-1">{request.prescription.refillsRemaining}</span>
                    </div>
                    <div>
                      <span className="font-medium text-gray-600">Last Filled:</span>
                      <span className="ml-1">
                        {request.prescription.lastFilledDate
                          ? new Date(request.prescription.lastFilledDate).toLocaleDateString()
                          : 'Never'}
                      </span>
                    </div>
                  </div>
              ...
              {request.denialReason && (
                <div className="mt-3 p-3 bg-red-50 rounded">
                  <p className="text-xs font-medium text-red-800 mb-1">Denial Reason:</p>
