...
export interface Report {
  ...
  name: string;
  description?: string;
  type: 'quality' | 'financial' | 'operational' | 'population' | 'custom' | 'regulatory';
  category?: string;
...
class ReportingService {
   ...
   * Create new report
   */
  async createReport(report: Omit<Report, 'id' | 'createdAt' | 'updatedAt' | 'lastRun' | 'nextRun'>): Promise<Report> {
   ...
   */
  async getReportExecutions(reportId: string, limit = 10): Promise<ReportExecution[]> {
  ...
  async pauseScheduledReport(reportId: string): Promise<void> {
    const response = await fetch(`${this.baseUrl}/scheduled/${reportId}/pause`, {
      method: 'POST',
    });
  ...
  async resumeScheduledReport(reportId: string): Promise<void> {
    const response = await fetch(`${this.baseUrl}/scheduled/${reportId}/resume`, {
      method: 'POST',
    });
    ...
    const regulatoryReports: Partial<Report>[] = [
      {
        ...
        type: 'regulatory',
        category: 'cms',
        description: 'Comprehensive CMS quality measures including core measures, HCAHPS, and HACs',
        format: 'pdf',
        status: 'active',
      },
      {
      ...
      },
      {
        ...
        type: 'regulatory',
        category: 'hedis',
        description: 'Healthcare Effectiveness Data and Information Set measures',
        format: 'pdf',
        status: 'active',
      },
      {
  ...
  validateReport(report: Partial<Report>): {
    ...
    if (!report.query || !report.query.metrics || report.query.metrics.length === 0) {
    ...
    if (report.schedule) {
      ...
      if (report.schedule.frequency === 'monthly' && report.schedule.dayOfMonth === undefined) {
   ...
   */
  calculateNextRun(schedule: ReportSchedule, fromDate: Date = new Date()): Date {
    ...
    switch (schedule.frequency) {
      ...
      case 'biweekly':
        ...
        if (daysToAddBiweekly <= 0 || (daysToAddBiweekly === 0 && nextRun <= fromDate)) {
