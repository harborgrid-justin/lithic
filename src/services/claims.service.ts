...
class ClaimsService {
  ...
  async searchCPTCodes(query: string): Promise<CPTCode[]> {
    const response = await fetch(`${this.apiBase}/coding/cpt?q=${encodeURIComponent(query)}`);
    if (!response.ok) throw new Error('Failed to search CPT codes');
    return response.json();
  }

  async searchICDCodes(query: string): Promise<ICDCode[]> {
    const response = await fetch(`${this.apiBase}/coding/icd?q=${encodeURIComponent(query)}`);
  ...
  getClaimAge(claim: Claim): number {
    ...
    return Math.floor((today.getTime() - dos.getTime()) / (1000 * 60 * 60 * 24));
  }
  ...
  filterClaimsByDateRange(claims: Claim[], startDate: string, endDate: string): Claim[] {
  ...
  calculateTotalCharges(claims: Claim[]): number {
    return claims.reduce((sum, claim) => sum + this.calculateClaimTotal(claim), 0);
  }
