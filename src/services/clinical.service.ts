...
export async function getClinicalNotes(
  ...
  const url = patientId
    ? `${API_BASE}/notes?patientId=${patientId}`
    : `${API_BASE}/notes`;
...
export async function getMedications(patientId: string): Promise<Medication[]> {
  const response = await fetch(
    `${API_BASE}/medications?patientId=${patientId}`,
  );
...
export async function getOrders(patientId?: string): Promise<Order[]> {
  const url = patientId
    ? `${API_BASE}/orders?patientId=${patientId}`
    : `${API_BASE}/orders`;
  const response = await fetch(url);
...
export async function searchICD10Codes(query: string): Promise<ICD10Code[]> {
  const response = await fetch(
    `${API_BASE}/icd10?q=${encodeURIComponent(query)}`,
  );
...
export async function searchCPTCodes(query: string): Promise<CPTCode[]> {
  const response = await fetch(
    `${API_BASE}/cpt?q=${encodeURIComponent(query)}`,
  );
