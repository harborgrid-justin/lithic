...
import type { AppointmentInput, WaitlistInput, ScheduleTemplateInput } from '@/lib/validators';

class SchedulingService {
  // Appointments
  async getAppointments(filters?: AppointmentFilters): Promise<Appointment[]> {
    const { data } = await api.get('/scheduling/appointments', { params: filters });
    return data;
  }
  ...
  async updateAppointment(id: string, appointment: Partial<AppointmentInput>): Promise<Appointment> {
    const { data } = await api.put(`/scheduling/appointments/${id}`, appointment);
    return data;
  }
  ...
  async cancelAppointment(id: string, reason?: string): Promise<Appointment> {
    const { data } = await api.post(`/scheduling/appointments/${id}/cancel`, { reason });
    return data;
  }

  async rescheduleAppointment(id: string, newStartTime: string): Promise<Appointment> {
    const { data } = await api.post(`/scheduling/appointments/${id}/reschedule`, { newStartTime });
    return data;
  }
  ...
  async completeAppointment(id: string, notes?: string): Promise<Appointment> {
    const { data } = await api.post(`/scheduling/appointments/${id}/complete`, { notes });
    return data;
  }
  ...
  async checkConflicts(
    ...
    const { data } = await api.post('/scheduling/appointments/check-conflicts', {
      providerId,
      startTime,
      duration,
  ...
  async createRecurringAppointments(appointment: AppointmentInput): Promise<RecurringSeries> {
  ...
  async getProviderSchedule(
    ...
    const { data } = await api.get(`/scheduling/providers/${providerId}/schedule`, {
  ...
  async getResourceSchedule(
    ...
    const { data } = await api.get(`/scheduling/resources/${resourceId}/schedule`, {
      params: { startDate, endDate },
    });
    return data;
  }
  ...
  // Waitlist
  async getWaitlist(filters?: { providerId?: string; status?: string }): Promise<WaitlistEntry[]> {
  ...
  async scheduleFromWaitlist(id: string, appointmentData: AppointmentInput): Promise<{
    appointment: Appointment;
    waitlistEntry: WaitlistEntry;
  }> {
    const { data } = await api.post(`/scheduling/waitlist/${id}/schedule`, appointmentData);
    return data;
  }
  ...
  async createScheduleTemplate(template: ScheduleTemplateInput): Promise<ScheduleTemplate> {
  ...
  async applyScheduleTemplate(
    ...
    const { data } = await api.post(`/scheduling/templates/${templateId}/apply`, {
      providerId,
      startDate,
      endDate,
    });
  ...
  async bulkCancelAppointments(appointmentIds: string[], reason?: string): Promise<void> {
  ...
  async searchProviders(query: string): Promise<Provider[]> {
    const { data } = await api.get('/scheduling/providers/search', { params: { q: query } });
    return data;
  }
  ...
  async getProviderStats(providerId: string, startDate: string, endDate: string) {
    const { data } = await api.get(`/scheduling/providers/${providerId}/stats`, {
      params: { startDate, endDate },
    });
    return data;
  }
