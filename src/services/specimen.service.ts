...
export class SpecimenService {
  ...
  static async createSpecimen(specimen: Omit<Specimen, 'id' | 'createdAt' | 'updatedAt'>): Promise<Specimen> {
  ...
  static async updateSpecimen(id: string, updates: Partial<Specimen>): Promise<Specimen> {
    const response = await fetch(`${this.baseUrl}/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  static async receiveSpecimen(id: string, receivedBy: string): Promise<Specimen> {
  ...
  static async updateStatus(id: string, status: SpecimenStatus): Promise<Specimen> {
  ...
  static validateSpecimen(specimen: Partial<Specimen>): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  static isSpecimenExpired(specimen: Specimen, maxAgeHours: number = 24): boolean {
