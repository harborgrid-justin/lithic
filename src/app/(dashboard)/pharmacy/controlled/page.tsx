...
import { pharmacyService, type ControlledSubstanceLog } from '@/services/pharmacy.service';
...
export default function ControlledSubstancePage() {
  ...
  const stats = {
    ...
    inventoryCount: logs.filter(l => l.transactionType === 'inventory-count').length,
  };
    ...
    <div className="p-8">
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Controlled Substance Log</h1>
            <p className="text-gray-600">Track all controlled substance transactions and inventory</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg border border-green-200 p-4">
            ...
            <div className="text-2xl font-bold text-green-600">{stats.receive}</div>
          </div>
          <div className="bg-white rounded-lg border border-blue-200 p-4">
            <div className="text-xs text-gray-600 mb-1">Dispensed</div>
            <div className="text-2xl font-bold text-blue-600">{stats.dispense}</div>
          </div>
          <div className="bg-white rounded-lg border border-orange-200 p-4">
            <div className="text-xs text-gray-600 mb-1">Wasted</div>
            <div className="text-2xl font-bold text-orange-600">{stats.waste}</div>
          </div>
          <div className="bg-white rounded-lg border border-purple-200 p-4">
            <div className="text-xs text-gray-600 mb-1">Transferred</div>
            <div className="text-2xl font-bold text-purple-600">{stats.transfer}</div>
          </div>
          <div className="bg-white rounded-lg border border-yellow-200 p-4">
            <div className="text-xs text-gray-600 mb-1">Inventory Counts</div>
            <div className="text-2xl font-bold text-yellow-600">{stats.inventoryCount}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          <div className="bg-red-50 rounded-lg border-2 border-red-200 p-4">
            ...
            <div className="text-2xl font-bold text-red-700">{scheduleBreakdown.scheduleII}</div>
          </div>
          <div className="bg-orange-50 rounded-lg border-2 border-orange-200 p-4">
            ...
            <div className="text-2xl font-bold text-orange-700">{scheduleBreakdown.scheduleIII}</div>
          </div>
          <div className="bg-yellow-50 rounded-lg border-2 border-yellow-200 p-4">
            ...
            <div className="text-2xl font-bold text-yellow-700">{scheduleBreakdown.scheduleIV}</div>
          </div>
          <div className="bg-green-50 rounded-lg border-2 border-green-200 p-4">
            ...
            <div className="text-2xl font-bold text-green-700">{scheduleBreakdown.scheduleV}</div>
          </div>
        </div>
      </div>
