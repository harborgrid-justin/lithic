...
export async function POST(request: NextRequest) {
  try {
    ...
    const sourceIndex = patientsDb.findIndex(p => p.id === mergeRequest.sourcePatientId);
    const targetIndex = patientsDb.findIndex(p => p.id === mergeRequest.targetPatientId);
    ...
    const mergedPatient: Patient = {
      ...
      insurance: mergeRequest.mergeInsurance
        ? [...(targetPatient.insurance || []), ...(sourcePatient.insurance || [])]
        : targetPatient.insurance,
      // Merge contacts if requested
      emergencyContacts: mergeRequest.mergeContacts
        ? [...(targetPatient.emergencyContacts || []), ...(sourcePatient.emergencyContacts || [])]
