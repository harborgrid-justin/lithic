...
import { webhookManager, WebhookEventType } from '../../integrations/webhooks/manager';
import {
...
router.post('/subscriptions', async (req: Request, res: Response) => {
  try {
    const { url, events, secret, headers, retryAttempts, timeout, active = true } = req.body;
    ...
    logger.error('Webhook subscription creation error', { error: error.message });
...
router.delete('/subscriptions/:id', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Webhook subscription deletion error', { error: error.message });
 ...
 */
router.get('/subscriptions/:id/deliveries', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
router.get('/events', async (req: Request, res: Response) => {
  try {
    const events: { event: WebhookEventType; description: string }[] = [
      ...
      { event: 'result.available', description: 'Lab/imaging result available' },
      ...
      { event: 'billing.claim.submitted', description: 'Billing claim submitted' },
