 ...
 * @access  Private
 */
router.post('/', pharmacyController.createRefillRequest.bind(pharmacyController));
 ...
 * @access  Private
 */
router.post('/:id/approve', pharmacyController.approveRefillRequest.bind(pharmacyController));
 ...
 * @access  Private
 */
router.post('/:id/deny', pharmacyController.denyRefillRequest.bind(pharmacyController));
...
router.get('/pending', async (req, res) => {
  try {
    ...
    await pharmacyController.getRefillRequests.call(pharmacyController, req, res);
  } catch (error) {
    res.status(500).json({
      success: false,
