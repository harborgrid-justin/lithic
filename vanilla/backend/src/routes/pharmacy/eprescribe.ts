 ...
 * @access  Private
 */
router.post('/', pharmacyController.processEPrescription.bind(pharmacyController));
 ...
 * @access  Private
 */
router.post('/:id/accept', pharmacyController.acceptEPrescription.bind(pharmacyController));
 ...
 * @access  Private
 */
router.post('/:id/reject', pharmacyController.rejectEPrescription.bind(pharmacyController));
...
router.get('/pending', async (req, res) => {
  try {
    ...
    await pharmacyController.getEPrescriptions.call(pharmacyController, req, res);
  } catch (error) {
    res.status(500).json({
      success: false,
