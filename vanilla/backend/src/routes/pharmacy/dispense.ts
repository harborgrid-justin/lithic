 ...
 * @access  Private
 */
router.post('/:prescriptionId', pharmacyController.dispensePrescription.bind(pharmacyController));
...
router.get('/queue', async (req, res) => {
  try {
    ...
    await pharmacyController.getPrescriptions.call(pharmacyController, req, res);
  } catch (error) {
    res.status(500).json({
      success: false,
