...
const updateReportSchema = z.object({
  reportType: z
    .enum(["PRELIMINARY", "FINAL", "ADDENDUM", "CORRECTION"])
    .optional(),
  ...
  comparison: z.string().optional(),
  status: z
    .enum(["DRAFT", "PRELIMINARY", "FINAL", "AMENDED", "CORRECTED"])
    .optional(),
  criticalResult: z.boolean().optional(),
});
 ...
 */
router.get("/study/:studyInstanceUID", authMiddleware, async (req, res) => {
  try {
    const report = await controller.getReportByStudyUID(
...
router.put(
  ...
  async (req, res) => {
    try {
      const report = await controller.updateReport(
        req.params.id,
        req.body,
        req.user,
      );
 ...
 */
router.post("/:id/correct", authMiddleware, async (req, res) => {
  try {
...
router.get("/templates/list", authMiddleware, async (req, res) => {
  try {
    const templates = await controller.getReportTemplates(
      req.query.modality as string,
    );
    res.json(templates);
  } catch (error) {
    res.status(500).json({
 ...
 */
router.post("/:id/voice-dictation", authMiddleware, async (req, res) => {
  try {
