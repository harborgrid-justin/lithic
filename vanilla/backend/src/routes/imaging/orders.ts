...
const createOrderSchema = z.object({
  ...
  bodyPart: z.string(),
  laterality: z.enum(['LEFT', 'RIGHT', 'BILATERAL', 'UNILATERAL', 'NA']).optional(),
...
const updateOrderSchema = z.object({
  status: z.enum(['PENDING', 'SCHEDULED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED', 'DISCONTINUED']).optional(),
  scheduledDateTime: z.string().datetime().optional(),
...
router.put(
  ...
  async (req, res) => {
    try {
      const order = await controller.updateOrder(req.params.id, req.body, req.user);
      res.json(order);
    } catch (error) {
      res.status(500).json({
