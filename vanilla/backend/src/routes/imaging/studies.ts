...
const updateStudySchema = z.object({
  studyDescription: z.string().optional(),
  readingStatus: z
    .enum(["UNREAD", "PRELIMINARY", "FINAL", "AMENDED", "DICTATED"])
    .optional(),
 ...
 */
router.get("/:studyInstanceUID", authMiddleware, async (req, res) => {
  try {
 ...
 */
router.get("/:studyInstanceUID/series", authMiddleware, async (req, res) => {
  try {
 ...
 */
router.get("/:studyInstanceUID/metadata", authMiddleware, async (req, res) => {
  try {
    const metadata = await controller.getStudyMetadata(
      req.params.studyInstanceUID,
    );
    res.json(metadata);
  } catch (error) {
 ...
 */
router.post("/:studyInstanceUID/compare", authMiddleware, async (req, res) => {
  try {
 ...
 */
router.get("/:studyInstanceUID/priors", authMiddleware, async (req, res) => {
  try {
 ...
 */
router.delete("/:studyInstanceUID", authMiddleware, async (req, res) => {
  try {
 ...
 */
router.post("/:studyInstanceUID/share", authMiddleware, async (req, res) => {
  try {
