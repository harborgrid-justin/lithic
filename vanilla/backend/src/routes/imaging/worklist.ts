...
router.put(
  ...
  async (req, res) => {
    try {
      const item = await controller.updateWorklistItem(
        req.params.id,
        req.body,
        req.user,
      );
 ...
 */
router.get("/date/today", authMiddleware, async (req, res) => {
  try {
    const worklist = await controller.getTodayWorklist(
...
router.get("/technician/:technicianId", authMiddleware, async (req, res) => {
  try {
    const worklist = await controller.getTechnicianWorklist(
 ...
 */
router.get("/radiologist/:radiologistId", authMiddleware, async (req, res) => {
  try {
    const worklist = await controller.getRadiologistWorklist(
      req.params.radiologistId,
    );
    res.json(worklist);
  } catch (error) {
    res.status(500).json({
