...
export class HL7Service {
  ...
  private static generateOBR(observationData: any): HL7Segment {
    return {
      ...
      fields: [
        ...
        "",
        this.formatHL7DateTime(
          observationData.observationDateTime || new Date(),
        ),
        this.formatHL7DateTime(
          observationData.collectionDateTime || new Date(),
        ),
        "",
        "",
        "",
        ...
        "",
        observationData.specimenReceivedDateTime
          ? this.formatHL7DateTime(observationData.specimenReceivedDateTime)
          : "",
        observationData.specimenSource || "",
        "",
        "",
  ...
  private static generateOBX(resultData: any, setId: number): HL7Segment {
    ...
    const referenceRange = resultData.referenceRange
      ? `${resultData.referenceRange.min}-${resultData.referenceRange.max}`
      : "";
  ...
  private static segmentToString(segment: HL7Segment): string {
    return (
      segment.segmentType +
      this.FIELD_SEPARATOR +
      segment.fields.join(this.FIELD_SEPARATOR)
    );
  }
  ...
  static generateOrderMessage(orderData: {
    ...
    // MSH
    segments.push(
      this.segmentToString(this.generateMSH("ORM^O01", messageControlId)),
    );
  ...
  static generateResultMessage(resultData: {
    ...
    // MSH
    segments.push(
      this.segmentToString(this.generateMSH("ORU^R01", messageControlId)),
    );
   ...
   */
  static validateHL7Message(message: string): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
  ...
  static generateACK(
    ...
    segments.push(
      this.segmentToString(this.generateMSH("ACK", messageControlId)),
    );

    // MSA segment
    segments.push(
      `MSA${this.FIELD_SEPARATOR}${ackCode}${this.FIELD_SEPARATOR}${originalMessageId}`,
    );
