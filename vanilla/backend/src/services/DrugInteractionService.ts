...
export class DrugInteractionService extends EventEmitter {
  ...
  async checkInteractions(
    ...
    // Clinical warnings
    if (patientData) {
      const warnings = await this.generateClinicalWarnings(
        newMedication,
        patientData,
      );
    ...
    const hasContraindications =
      result.interactions.some((i) => i.severity === "contraindicated") ||
      result.allergyAlerts.some(
        (a) => a.hasAllergy && a.severity === "severe",
      ) ||
      result.diseaseContraindications.some((d) => d.severity === "absolute");

    const hasMajorIssues =
      result.interactions.some((i) => i.severity === "major") ||
      result.allergyAlerts.some(
        (a) => a.hasAllergy && a.severity === "moderate",
      ) ||
      result.clinicalWarnings.some((w) => w.severity === "contraindicated");

    result.safe = !hasContraindications;
    result.requiresPrescriberNotification =
      hasContraindications || hasMajorIssues;
    ...
    for (const allergy of allergies) {
      ...
      if (
        ...
        alerts.push({
          ...
          severity: "severe",
          recommendation:
            "DO NOT DISPENSE - Patient has documented allergy to this medication",
        });
        continue;
      }
      ...
      for (const [drugClass, members] of this.drugClassMappings.entries()) {
        if (allergyLower.includes(drugClass.toLowerCase())) {
          ...
          if (
            members.some((member) =>
              medNameLower.includes(member.toLowerCase()),
            )
          ) {
            alerts.push({
              hasAllergy: true,
      ...
      nsaid: [
        {
          ...
          recommendation:
            "Use with caution. Consider alternative therapy or add gastric protection.",
        },
        {
          diseaseCode: "N18",
          ...
          recommendation:
            "Avoid if possible. Monitor renal function closely if used.",
        },
      ],
      "beta-blocker": [
        {
          ...
          severity: "absolute",
          description:
            "Beta-blockers can cause bronchospasm in asthma patients",
          recommendation:
            "DO NOT USE. Select alternative antihypertensive agent.",
        },
      ],
    };
   ...
   */
  private async findFoodInteractions(
    medication: Medication,
    ...
    if (medNameLower.includes("warfarin")) {
      interactions.push({
        ...
        recommendation:
          "Maintain consistent vitamin K intake. Avoid large variations.",
      });
    }
    ...
    if (
      ...
      (medNameLower.includes("tetracycline") ||
        medNameLower.includes("fluoroquinolone"))
    ) {
      interactions.push({
        ...
        recommendation:
          "Take 2 hours before or 6 hours after dairy/supplements.",
      });
    }
  ...
  private async generateClinicalWarnings(
    ...
    if (patientData.isPregnant) {
      ...
      if (
        ...
        warnings.push({
          ...
          description:
            "This medication is contraindicated in pregnancy (Category X)",
      ...
      // ACE inhibitors/ARBs
        ...
        warnings.push({
          ...
          severity: "contraindicated",
          description:
            "ACE inhibitors/ARBs can cause fetal harm in 2nd and 3rd trimester",
          recommendation:
            "Discontinue as soon as pregnancy detected. Contact prescriber.",
        });
      }
    }
    ...
    if (patientData.isLactating && medication.isControlled) {
      warnings.push({
        ...
        recommendation:
          "Counsel patient on risks. Consider alternative if possible.",
      });
    }
    ...
    if (patientData.age && patientData.age >= 65) {
      ...
      if (
        ...
        warnings.push({
          ...
          severity: "caution",
          description:
            "This medication is potentially inappropriate in elderly (Beers Criteria)",
          recommendation:
            "Use lowest effective dose. Monitor for adverse effects closely.",
        });
      }
    }
    ...
    if (patientData.age && patientData.age < 18) {
      if (medNameLower.includes("tetracycline")) {
        warnings.push({
          ...
          severity: "contraindicated",
          description:
            "Tetracyclines can cause permanent tooth discoloration in children",
          recommendation:
            "Generally avoid in children under 8 years. Contact prescriber.",
        });
      }
    }
    ...
    if (patientData.renalFunction && patientData.renalFunction !== "normal") {
      if (
        ...
        warnings.push({
          type: "renal",
          severity:
            patientData.renalFunction === "severe"
              ? "contraindicated"
              : "warning",
          description: `Dose adjustment required for renal impairment (${patientData.renalFunction})`,
          recommendation:
            "Verify dose is appropriate for renal function. May need reduction.",
        });
      }
    }
    ...
    if (
      ...
      patientData.hepaticFunction !== "normal"
    ) {
      if (
        ...
        warnings.push({
          type: "hepatic",
          severity:
            patientData.hepaticFunction === "severe" ? "warning" : "caution",
          description: `Caution in hepatic impairment (${patientData.hepaticFunction})`,
          recommendation:
            "Monitor liver function. May need dose adjustment or alternative.",
        });
      }
    }
    ...
    const commonInteractions: Omit<DrugInteraction, "id" | "createdAt">[] = [
      {
        ...
        management:
          "Avoid combination if possible. If必要, monitor INR closely and watch for signs of bleeding.",
      ...
      },
      {
        ...
        interactionType: "drug-drug",
        description:
          "Severe increase in statin levels with risk of rhabdomyolysis",
        clinicalEffects:
          "Muscle pain, weakness, rhabdomyolysis, acute renal failure",
        mechanism: "Clarithromycin inhibits CYP3A4 metabolism of simvastatin",
        management:
          "DO NOT COMBINE. Discontinue statin during clarithromycin therapy or use alternative antibiotic.",
      ...
      },
      {
        ...
        management:
          "Monitor potassium levels closely. Use combination only if necessary.",
