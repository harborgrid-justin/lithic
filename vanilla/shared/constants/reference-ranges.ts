...
export function getReferenceRange(loincCode: string, age: number, gender: 'male' | 'female'): ReferenceRange | undefined {
  const ageGroup = age < 1 ? 'infant' : age < 18 ? 'pediatric' : 'adult';

  return REFERENCE_RANGES.find(range =>
    range.loincCode === loincCode &&
    (range.ageGroup === ageGroup || range.ageGroup === 'all') &&
    (range.gender === gender || range.gender === 'all')
  );
}

export function isCritical(loincCode: string, value: number, age: number, gender: 'male' | 'female'): boolean {
...
export function isAbnormal(loincCode: string, value: number, age: number, gender: 'male' | 'female'): boolean {
  const range = getReferenceRange(loincCode, age, gender);
  if (!range) return false;

  if (range.minValue !== undefined && value < range.minValue) return true;
  if (range.maxValue !== undefined && value > range.maxValue) return true;

  return false;
}

export function getAbnormalityFlag(loincCode: string, value: number, age: number, gender: 'male' | 'female'): 'L' | 'H' | 'LL' | 'HH' | 'N' {
  if (isCritical(loincCode, value, age, gender)) {
    const range = getReferenceRange(loincCode, age, gender);
    if (range?.criticalLow !== undefined && value < range.criticalLow) return 'LL';
    if (range?.criticalHigh !== undefined && value > range.criticalHigh) return 'HH';
  }

  if (isAbnormal(loincCode, value, age, gender)) {
    const range = getReferenceRange(loincCode, age, gender);
    if (range?.minValue !== undefined && value < range.minValue) return 'L';
    if (range?.maxValue !== undefined && value > range.maxValue) return 'H';
  }
