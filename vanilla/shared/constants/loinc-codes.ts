...
export interface LOINCCode {
  ...
  relatedNames?: string[];
  unit?: string;
  category: 'chemistry' | 'hematology' | 'microbiology' | 'immunology' | 'pathology' | 'molecular' | 'toxicology' | 'endocrinology' | 'coagulation';
}
...
export function getLOINCCodesByCategory(category: LOINCCode['category']): LOINCCode[] {
  return Object.values(LOINC_CODES).filter(loinc => loinc.category === category);
}

export function searchLOINCCodes(query: string): LOINCCode[] {
  const lowerQuery = query.toLowerCase();
  return Object.values(LOINC_CODES).filter(loinc =>
