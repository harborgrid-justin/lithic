...
export class AppointmentDetailPage {
  ...
  async render(): Promise<void> {
    try {
      const appointment = await this.schedulingService.getAppointment(
        this.appointmentId,
      );
      ...
      this.container.innerHTML =
        '<div class="error">Failed to load appointment details</div>';
    }
  }
  ...
  private attachEventListeners(appointment: any): void {
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        ...
        if (reason) {
          await this.schedulingService.cancelAppointment(
            this.appointmentId,
            reason,
          );
