...
export class PopulationPage {
  ...
  private renderKPIs(metrics: any[]): void {
    const kpiGrid = this.container.querySelector('#population-kpis') as HTMLElement;
    if (!kpiGrid) return;
  ...
  private renderCharts(metrics: any[]): void {
    const chartContainer = this.container.querySelector('#population-charts') as HTMLElement;
    ...
    if (metric?.stratification?.byRiskLevel) {
      new ChartWidget(chartContainer, {
        ...
        data: {
          series: [{
            name: 'Risk Levels',
            data: Object.entries(metric.stratification.byRiskLevel).map(([level, count]) => ({
              x: level,
              y: count as number,
            })),
          }],
  ...
  private renderTable(metrics: any[]): void {
    const tableContainer = this.container.querySelector('#population-table') as HTMLElement;
