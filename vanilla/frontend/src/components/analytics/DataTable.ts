...
export class DataTable {
  ...
  private createPagination(): HTMLElement {
    ...
    for (let i = 1; i <= Math.min(totalPages, 5); i++) {
      const pageBtn = this.createButton(String(i), false, i === this.currentPage);
  ...
  private createButton(text: string, disabled: boolean, active: boolean = false): HTMLButtonElement {
