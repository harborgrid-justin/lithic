...
export class InsuranceCard {
  ...
  private renderInsuranceCard(insurance: Insurance): string {
    const effectiveDate = new Date(insurance.effectiveDate).toLocaleDateString();
  ...
  private attachListEventListeners(): void {
    const addBtn = this.container.querySelector('#addInsuranceBtn') as HTMLButtonElement;
    if (addBtn) {
    ...
    editButtons.forEach(button => {
      button.addEventListener('click', (e) => {
        ...
        if (insuranceId) {
          const insurance = this.insuranceList.find(ins => ins.id === insuranceId);
          if (insurance) {
            this.editMode = true;
  ...
  private attachFormEventListeners(): void {
    const form = this.container.querySelector('#insuranceForm') as HTMLFormElement;
    const cancelBtn = this.container.querySelector('#cancelFormBtn') as HTMLButtonElement;
  ...
  private async handleSubmit(): Promise<void> {
    ...
    try {
      const insuranceData: Insurance = {
        ...
        copay: formData.get('copay') ? parseFloat(formData.get('copay') as string) : undefined,
        deductible: formData.get('deductible') ? parseFloat(formData.get('deductible') as string) : undefined,
      };

      const response = await PatientService.updateInsurance(this.patientId, insuranceData);
