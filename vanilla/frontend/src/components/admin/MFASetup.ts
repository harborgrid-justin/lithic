...
export class MFASetup {
      ...
      if (codesContainer) {
        codesContainer.innerHTML = `
          <div class="backup-codes">
            ${response.backupCodes.map((code: string) => `
  ...
  private async enableMFA(): Promise<void> {
    const tokenInput = document.getElementById('verify-token') as HTMLInputElement;
  ...
  private async disableMFA(): Promise<void> {
    const tokenInput = document.getElementById('disable-token') as HTMLInputElement;
    const token = tokenInput?.value;
